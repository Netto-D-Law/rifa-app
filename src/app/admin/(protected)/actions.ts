...
//
// Importante: a tabela raffle_numbers não permite UPDATE direto por RLS
// (só através de funções do banco). Por isso, em vez de criar uma nova
// função SQL, reaproveitamos a função "liberar_pedido" já existente
// (mesma usada no botão individual de liberar um pedido), chamando-a
// uma vez para cada pedido ativo da rifa. O resultado final é idêntico
// a uma função de reset dedicada, sem precisar alterar o banco.
export async function resetarRifaAction(raffleId: string) {
  const supabase = await createClient();

  // Pega todos os pedidos que ainda têm números presos (pendentes ou
  // confirmados) — são exatamente os que precisam ser liberados.
