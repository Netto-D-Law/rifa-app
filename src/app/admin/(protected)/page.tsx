...
import BackupAutomatico from '@/components/admin/BackupAutomatico';
...
export default async function DashboardPage() {
  ...
  // Para o backup/exportação em Excel, buscamos TODOS os pedidos (sem o
  // limite de 20 usado só na listagem visual de "confirmados recentemente").
  ...
  // Prioriza o registro permanente de números (numeros_atribuidos) sobre
  // o vínculo ao vivo — assim os números continuam visíveis mesmo se o
  // pedido for liberado depois (auditoria).
  function numerosDoPedido(pedidoId: string, numerosAtribuidos: number[] | null): number[] {
    if (numerosAtribuidos && numerosAtribuidos.length > 0) return numerosAtribuidos;
    return numerosPorPedido.get(pedidoId) ?? [];
  }
                ...
                {pedidosConfirmados.map((p) => (
                  <tr key={p.id} className="border-t border-slate-100">
                    <td className="px-4 py-2.5 font-semibold">
                      {numerosDoPedido(p.id, p.numeros_atribuidos)
