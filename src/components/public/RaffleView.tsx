...
export default function RaffleView({
  ...
  function handleSetModoCarrinho(modo: 'manual' | 'quantidade') {
    ...
    // Ao trocar para "escolher na grade", muda automaticamente para a aba
    // "Todos os números" — sem isso, a pessoa desliza o botão mas a grade
    // não fica visível em lugar nenhum da tela.
    if (modo === 'manual') setTab('todos');
  ...
  return (
    <main className="min-h-screen pb-24">
      ...
      >
        <div className="mx-auto grid max-w-6xl items-center gap-6 md:grid-cols-[300px_1fr_220px]">
          ...
          <div className="grid overflow-hidden rounded-2xl bg-white shadow-xl md:grid-cols-[220px_1fr]">
            <div className="group relative flex min-h-[180px] items-center justify-center overflow-hidden bg-rotary-bluePale">
              {raffle.imagem_url ? (
                <>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={raffle.imagem_url}
                    alt={raffle.titulo}
                    onClick={() => setImagemAmpliada(true)}
                    className="h-full w-full cursor-pointer object-cover transition hover:brightness-90"
                  />
                  {/* Dica de "ver imagem completa" — só aparece no hover em
                      telas maiores (no celular a imagem já ocupa a largura
                      toda e não precisa de zoom). */}
                  <button
                    onClick={() => setImagemAmpliada(true)}
                    className="pointer-events-none absolute inset-0 hidden items-center justify-center bg-black/0 text-white opacity-0 transition group-hover:pointer-events-auto group-hover:bg-black/30 group-hover:opacity-100 md:flex"
                    aria-label="Ver imagem completa"
                  >
                    <span className="rounded-full bg-black/60 px-3 py-1.5 text-xs font-semibold">
                      🔍 Ver imagem completa
                    </span>
                  </button>
                </>
      ...
      {/* Lightbox — mostra a arte inteira, sem o corte do card. Fecha ao
          clicar fora ou no X. */}
      {imagemAmpliada && raffle.imagem_url && (
        <div
