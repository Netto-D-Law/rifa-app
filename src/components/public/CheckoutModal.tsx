...
export default function CheckoutModal({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (clube.trim().length < 2) {
      setErro('Informe o clube Rotary ao qual você pertence, para o repasse da verba arrecadada.');
      return;
    }
    ...
    setCarregando(true);
    // O clube do comprador e (quando pagamento é no cartão) o nome/contato
    // do titular são anexados ao campo "nome" enviado ao banco, já que
    // ainda não temos colunas próprias para isso — assim tudo fica
    // registrado e visível para o admin sem precisar alterar o banco.
    let nomeFinal = `${nome.trim()} — Clube: ${clube.trim()}`;
    if (metodo === 'cartao' && titularNome.trim()) {
      nomeFinal += ` (titular do cartão: ${titularNome.trim()} - ${titularContato.trim()})`;
    }
        ...
        {!resultado ? (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">
                  Clube Rotary a que você pertence
                </label>
                <input
                  value={clube}
                  onChange={(e) => setClube(e.target.value)}
                  placeholder="Ex.: Rotary Club Butantã"
                  className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-rotary-blue focus:outline-none"
                />
                <p className="mt-1 text-[11px] text-slate-400">
                  Usado para repassar parte da verba arrecadada ao seu clube.
                </p>
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">
          ...
          <div>
            ...
            {metodo === 'pix' ? (
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-slate-400">
                  Importante: leve o valor de {formatMoeda(resultado.valor_total)} para pagar na
                  maquininha.
                </p>
              </div>
            )}
